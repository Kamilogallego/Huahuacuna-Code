...
// Dinero: tiene monto y metodo
export type DonacionDinero = DonacionBase & {
  tipo: 'dinero';
  monto: number;
  metodo: string;
};

// Especie: tiene descripcion
export type DonacionEspecie = DonacionBase & {
  tipo: 'especie';
  descripcion: string;
};

// Union
export type Donacion = DonacionDinero | DonacionEspecie;

// Mock de solicitudes (lo dejé igual que antes)
...
// Mock de donaciones usando el discriminated union (SIN poner as const al array para evitar readonly en el nivel del array)
export const donacionesMock: Donacion[] = [
